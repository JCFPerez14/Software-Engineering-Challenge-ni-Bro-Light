...
// If the user is already logged in, check their account type and redirect accordingly
 if (isset($_SESSION['username']) && isset($_SESSION['account_type'])) {
  if ($_SESSION['account_type'] == 0) {
    header('location:index.php?status=daddy');
  } else if ($_SESSION['account_type'] == 1) {
    header('location:user_account.php?status=daddy');
  }
  exit();
 }
    ...
    <?php if (!empty($error)) : ?>
                        ...
                        <div class="container">
                          <div class="row gx-1">
                            ...
                            <div class="col">
                            <a type="submit" class="btn btn-danger btn-block" href="Sign_Up.php">Sign Up</a>
                            </div>
                          </div>
                        </div>
